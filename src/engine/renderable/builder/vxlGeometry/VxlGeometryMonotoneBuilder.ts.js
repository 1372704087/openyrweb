// === Reconstructed SystemJS module: engine/renderable/builder/vxlGeometry/VxlGeometryMonotoneBuilder ===
    ...
    return {
      ...
      execute: function () {
        (e(
          ...
          (i = class {
            build(e, t = !1) {
                    ...
                    for (l[s] = 1, o[s] = -1; o[s] < t[s];) {
                      ...
                      for (o[n] = 0; o[n] < t[n]; ++o[n]) {
                        ...
                        for (o[a] = 0; o[a] < t[a]; ++o[a], v = b) {
                          var S = 0 <= o[s] ? e(o[0], o[1], o[2]) : 0,
                            w = o[s] < t[s] - 1 ? e(o[0] + l[0], o[1] + l[1], o[2] + l[2]) : 0;
                          (!(b = S) == !w ? (b = 0) : S || (b = -w), v !== b && ((c[T++] = o[a]), (c[T++] = b)));
                        }
                        c[T++] = t[a];
                        for (var E = (c[T++] = 0), C = 0, x = 0; C < y && x < T - 2;) {
                          let e = f[h[C]];
                          var O = e.left[e.left.length - 1][0],
                            A = e.right[e.right.length - 1][0],
                            M = e.color,
                            R = c[x],
                            P = c[x + 2],
                            I = c[x + 1];
                          O < P && R < A && I === M
                            ? (e.merge_run(o[n], R, P), (u[E++] = h[C]), ++C, (x += 2))
                            : (P <= A && (I && ((k = new Y(I, o[n], R, P)), (u[E++] = f.length), f.push(k)), (x += 2)),
                              A <= P && (e.close_off(o[n]), ++C));
                        }
                        ...
                        for (; x < T - 2; x += 2) {
                          var k,
                            R = c[x],
                            P = c[x + 2];
                          (I = c[x + 1]) && ((k = new Y(I, o[n], R, P)), (u[E++] = f.length), f.push(k));
                        }
                        var B = u,
                          u = h,
                          h = B,
                      ...
                      for (C = 0; C < f.length; ++C) {
                        ...
                        for (x = 0; x < N.left.length; ++x) {
                          ...
                          var L = [0, 0, 0],
                            D = N.left[x];
                          ((L[s] = o[s]), (L[a] = D[0]), (L[n] = D[1]), i.push({ position: L, value: b }));
                        }
                        ...
                        for (
                          ...
                          U === N.left.length
                            ? (z = !0)
                            : H !== N.right.length && ((V = N.left[U]), (W = N.right[H]), (z = V[1] > W[1]));
                          var K = z ? g[H] : d[U],
                            q = z ? N.right[H] : N.left[U];
                          if (z !== G)
                            for (; F + 3 < _;)
                              (j === z ? r.push([p[F], p[F + 3], K]) : r.push([p[F + 3], p[F], K]), (F += 3));
                          else
                            for (; F + 3 < _;) {
                              for (x = 0; x < 2; ++x)
                                for (var $ = 0; $ < 2; ++$) m[x][$] = p[_ - 3 * (x + 1) + $ + 1] - q[$];
                              var Q = m[0][0] * m[1][1] - m[1][0] * m[0][1];
                              if (z === 0 < Q) break;
                              (0 != Q && (j === z ? r.push([p[_ - 3], p[_ - 6], K]) : r.push([p[_ - 6], p[_ - 3], K])),
                                (_ -= 3));
                            }
                          ((p[_++] = K), (p[_++] = q[0]), (p[_++] = q[1]), z ? ++H : ++U, (G = z));
                        }
                      }
                    }
                  }
              ...
              return (
                ...
                t || v.addAttribute("normal", new THREE.BufferAttribute(c, 3)),
                v.addAttribute("color", new THREE.BufferAttribute(h, 3)),
                v.setIndex(new THREE.BufferAttribute(f, 1)),
                (v = C.BufferGeometryUtils.mergeVertices(v)),
          ...
          (Y = class {
            constructor(e, t, i, r) {
              ((this.color = e), (this.left = [[i, t]]), (this.right = [[r, t]]));
            }
            close_off(e) {
