// === Reconstructed SystemJS module: game/trigger/executor/UnrevealAroundWaypointExecutor ===
          ...
          execute(e) {
            ...
            if (i)
              for (var r of e.getCombatants())
                e.mapShroudTrait.getPlayerShroud(r)?.unrevealAround(i, e.rules.general.revealTriggerRadius);
            else
              console.warn(`No valid location found for waypoint ${t}. ` + `Skipping action ${this.getDebugName()}.`);
          }
        }),
          e("UnrevealAroundWaypointExecutor", r));
      },
    };
  },
);
