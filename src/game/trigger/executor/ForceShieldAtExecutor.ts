/**
 * ForceShieldAtExecutor — 在路径点施加力场盾（动作 112）。
 *
 * 路径点 params[6]；对触发 house 在该格 activateEffect(ForceShield)。
 * 路径点无效 warn；house 无效或规则缺失则静默跳过（与孪生短路
 * 表达式一致）。
 *
 * 由 game/trigger/executor/ForceShieldAtExecutor.ts.js 重写为 TS
...
export class ForceShieldAtExecutor extends TriggerExecutor {
  /** 在路径点对触发 house 激活力场盾效果。 */
    ...
    if (tile) {
      ...
      if (
        (rule = [...world.rules.superWeaponRules.values()].find(
          (r: any) => r.type === SuperWeaponType.ForceShield,
        )) &&
        rule
      )
